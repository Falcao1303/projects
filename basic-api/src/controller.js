    ...
    events: function events(req, res, account_bank) {
        ...
        (body.type == 'deposit') ? (account_bank.length > 0) ?
            ...
            : (body.type == 'withdraw') ? (account_bank.length > 0) ?
                account_bank.map(element => {
                        (element.destination == body.origin) && ((element.amount - body.amount) >=  -100) ? (element.amount -= body.amount) & (res.status(201).send({ origin: { id: body.origin, balance: element.amount } })) : res.status(404).json(0)
                }) : res.status(404).json(0)
